...
async function shippo(
  ...
  if (!shippoKey) {
    throw new Error(
      "Shippo is not configured. Add SHIPPO_API_TOKEN in Supabase secrets.",
    );
  }
  ...
  try {
    ...
    if (action === "create_checkout") {
      ...
      const { data: order, error } = await sb
        .from("orders")
        .select(
          "*,products(*),buyer:profiles!orders_buyer_id_fkey(shipping_address),seller:profiles!orders_seller_id_fkey(shipping_address,stripe_account_id)",
        )
        .eq("id", body.orderId)
        .eq("buyer_id", user.id)
        .single();
      ...
      const shipping = Number(rates[0].amount);
