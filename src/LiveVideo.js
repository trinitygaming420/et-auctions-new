...
export default function LiveVideo({ room, user, host=false }) {
  ...
  return <LiveKitRoom serverUrl={serverUrl} token={token} connect audio={false} video={false} onError={e=>setError(e?.message||String(e))} onMediaDeviceFailure={e=>setError(`Camera or microphone failed: ${String(e||"unknown error")}`)}><Publisher host={host} onError={setError}/><Tracks host={host}/>{host?<CameraMenu onError={setError}/>:null}</LiveKitRoom>;
}
...
function CameraMenu({onError}){
  ...
  return <View style={styles.cameraMenu} pointerEvents="box-none">
    <Pressable onPress={switchCamera} disabled={switching} style={({pressed})=>[styles.switchButton,(pressed||switching)&&styles.switchPressed]}>
      <Text style={styles.switchIcon}>⟳</Text><Text style={styles.switchText}>{switching?"Switching…":"Switch camera"}</Text>
    </Pressable>
  </View>;
}
const styles={
  video:{width:"100%",height:390,backgroundColor:"#050609",alignItems:"center",justifyContent:"center",padding:25},
  cameraMenu:{position:"absolute",left:0,right:0,bottom:0,height:64,backgroundColor:"rgba(5,6,9,0.9)",borderTopWidth:1,borderTopColor:"#2a3040",alignItems:"center",justifyContent:"center",paddingHorizontal:18},
  switchButton:{minWidth:190,height:44,backgroundColor:"#1a1f2c",borderWidth:1,borderColor:"#3b4355",borderRadius:22,flexDirection:"row",alignItems:"center",justifyContent:"center",gap:9,paddingHorizontal:18},
  switchPressed:{opacity:0.55},
  switchIcon:{color:"#fff",fontSize:24,fontWeight:"800"},
