...
export default function LiveVideo({ room, user, host=false }) {
  ...
  return <LiveKitRoom serverUrl={serverUrl} token={token} connect audio={false} video={false} onError={e=>setError(e.message)} onMediaDeviceFailure={f=>setError(`Camera or microphone failed: ${String(f||"unknown device error")}`)}><Publisher host={host} onError={setError}/><Tracks host={host}/></LiveKitRoom>;
}
function Publisher({host,onError}){
  ...
  useEffect(()=>{
    ...
    (async()=>{
      ...
      }catch(e){if(active)onError(e?.message||"Could not start camera and microphone.")}
    })();
    return()=>{active=false};
