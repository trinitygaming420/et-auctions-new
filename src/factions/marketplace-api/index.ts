...
Deno.serve(async req=>{
  ...
  try{
    ...
    if(action==="connect_onboarding"){
      ...
      if(!account){const created=await stripe("accounts","POST",{type:"express",country:"US",email:user.email||"","metadata[user_id]":user.id});account=created.id;await sb.from("profiles").update({stripe_account_id:account}).eq("id",user.id);}
      const link=await stripe("account_links","POST",{account,refresh_url:returnUrl,return_url:returnUrl,type:"account_onboarding"});
      return json({url:link.url});
    }
    if(action==="connect_status"){
      if(!profile.stripe_account_id)return json({complete:false,payoutsEnabled:false});
      const account=await stripe(`accounts/${profile.stripe_account_id}`);
      const complete=!!account.details_submitted,payoutsEnabled=!!account.payouts_enabled;
      await sb.from("profiles").update({stripe_onboarding_complete:complete,payouts_enabled:payoutsEnabled}).eq("id",user.id);
      return json({complete,payoutsEnabled});
    }
