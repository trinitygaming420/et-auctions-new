-- Scheduled shows and preloaded auction inventory
...
create index if not exists shows_schedule_idx
  on public.shows(seller_id, status, starts_at);

create index if not exists products_show_idx
  on public.products(show_id, created_at);
...
create policy "public show media"
on storage.objects
for select
...
create policy "seller uploads show media"
on storage.objects
for insert
to authenticated
with check (
  bucket_id = 'show-media'
